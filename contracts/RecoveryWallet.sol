...
contract RecoveryWallet is UsedPrecompiles {
    ...
    modifier notBlocked() {
        require(!blocked, "the transfers are blocked");
        _;
    }
    ...
    function transfer(address _tokenAddr, address _to, uint256 _amount) external notBlocked onlyOwner returns (bool) {
    ...
    function propose(address _target, uint256 _value, bytes calldata _data) external onlyWallet returns (uint256) {
        ...
        proposals[id].value = _value;
        proposals[id].data = _data;
    ...
    function proposeUnBlock() external onlyOwnerOrAdmin {
        bytes memory data = abi.encodeWithSignature("unBlock()");
        uint256 id = this.propose(address(this), 0, data);
        emit SetOwnerProposed(id, msg.sender);
    }

    function unBlock() public onlyWallet {
        blocked = false;
    }

    function blockTransfers() public onlyOwnerOrAdmin {
        blocked = true;
    }
}